...
/*
  The circuit:
    SD card reader and ArduCam attached to SPI bus as follows:
 ** MOSI - pin 11 on Arduino Uno
 ** MISO - pin 12 on Arduino Uno
 ** CLK - pin 13 on Arduino Uno
 ** CS - Optional, in this example pin 4 is used for the SD card and 
         pin 7 is used for the ArduCam
 ** ArduCam Only:
 ** SDA - SDA pin on Arduino Uno
 ** SCL - SCL pin on Arduino Uno
 
     *The MOSI, MISO, AND CLK(or SCK) for the SD card reader and ArduCam
     *should be wired in series.
     *The 3.3V power supply should be wired in parallel to the two modules
*/
...
void setup() {
  ...
  while (1) {
    ...
    myCAM.rdSensorReg16_8(OV2640_CHIPID_HIGH, &vid);
    myCAM.rdSensorReg16_8(OV2640_CHIPID_LOW, &pid);
