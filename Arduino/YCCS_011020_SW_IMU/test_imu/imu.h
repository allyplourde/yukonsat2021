...
/** Class for hardware interfacing with an IMU */
class IMU{
public:
  IMU(void);
  ...
  void reset(uint8_t Ascale, uint8_t Gscale, uint8_t AODR, uint8_t GODR);
  void readData();
  void status();

  int16_t t0,
          ax,         ///< The last read X mag in raw units
          ay,         ///< The last read Y mag in raw units
          az,         ///< The last read Z mag in raw units
          gx,         ///< The last read X mag in raw units
          gy,         ///< The last read Y mag in raw units
          gz;         ///< The last read Z mag in raw units
